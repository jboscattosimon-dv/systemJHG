...
-- ================================================================
-- Nova forma de pagamento 'parcelado' na tela de Vendas: em vez de
-- lançar o valor cheio no caixa na hora (como as outras formas fazem),
-- gera N parcelas em contas_receber, uma por mês, ligadas à comanda.
--
-- CREATE OR REPLACE não troca a função quando a lista de parâmetros
-- muda — cria uma segunda (overload) e quebra tudo via PGRST203 (já
-- aconteceu antes nesse projeto). Por isso o DROP explícito da versão
-- de 4 parâmetros antes de recriar com 5.
...
BEGIN
  ...
  IF p_forma_pagamento = 'parcelado' THEN
    -- Dinheiro só entra de verdade quando cada parcela for recebida (via
    -- baixar_conta_receber) — não lança nada em movimentos_caixa agora.
    ...
    FOR i IN 1..v_parcelas LOOP
      ...
      ELSE
        v_valor_parcela_i := v_total - v_soma_parcelas;
      END IF;
