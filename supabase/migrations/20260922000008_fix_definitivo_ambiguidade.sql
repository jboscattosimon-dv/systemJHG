-- ================================================================
--  Fix definitivo: ON CONFLICT (produto_id, tamanho) é ambíguo
-- ================================================================
-- As duas correções anteriores (produto_id e preco_venda_prazo nas
-- consultas WHERE/SET) estavam certas, mas sobrava um terceiro caso
-- que não dá pra corrigir qualificando a tabela: o alvo do
-- "ON CONFLICT (produto_id, tamanho)" não aceita "produto_tamanhos.produto_id"
-- ali dentro — é erro de sintaxe. Como produto_id também é nome de
-- coluna do RETURNS TABLE da função, o Postgres continuava vendo
-- ambiguidade nessa cláusula.
--
-- A solução correta é a diretiva do PL/pgSQL "#variable_conflict
-- use_column", que instrui a função inteira a sempre preferir a
-- coluna da tabela sobre a variável/parâmetro de saída sempre que
-- houver esse conflito de nome — resolve de vez qualquer ocorrência,
-- inclusive essa que não dava pra qualificar manualmente.
...
BEGIN
  ...
  LOOP
    ...
    -- Preço manual do item tem prioridade; sem ele, cai no cálculo por margem.
    v_preco_vista := CASE
  ...
  -- Lançamento financeiro da entrada inteira — saída sem depender de
  -- caixa aberto (sessao_caixa_id fica NULL, a tela de Financeiro lista
  -- todo mundo, só o "Movimentos da sessão" do Caixa que filtra por sessão).
  ...
  VALUES ('saida', 'compra_mercadoria', v_descricao, v_total_entrada, NULL, v_empresa);
END;
$$;
