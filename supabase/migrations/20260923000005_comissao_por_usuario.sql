...
--  gerar comissão pro profissional do item E pro atendente que
--  fechou a venda, se ambos tiverem percentual configurado.
-- ================================================================
...
RETURNS TABLE (
  ...
  telas_permitidas TEXT[], comissao_percentual NUMERIC
)
LANGUAGE sql SECURITY DEFINER SET search_path = public STABLE
AS $$
  SELECT au.id, au.email, up.papel, up.profissional_id, up.ativo, au.created_at, up.empresa_id, e.nome, up.telas_permitidas, up.comissao_percentual
  FROM auth.users au
...
BEGIN
  ...
  VALUES (p_cliente_id, COALESCE(NULLIF(p_cliente_nome, ''), 'Balcão'), 'fechada', v_total_final, p_forma_pagamento, v_empresa, auth.uid())
  ...
  IF v_comissao_usuario IS NOT NULL AND v_comissao_usuario > 0 THEN
    ...
    VALUES (auth.uid(), v_comanda_id, v_total_final, v_comissao_usuario, ROUND(v_total_final * v_comissao_usuario / 100, 2), 'pendente', v_empresa);
  END IF;
